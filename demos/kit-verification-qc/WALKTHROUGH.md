# Kit Verification QC — live walkthrough

**Audience:** the prospect's manufacturing engineer and whoever they bring from IT/leadership. **Runs on:** http://localhost:3004 (`kitqc@example.com / demo123`). **Length:** 12–15 minutes. **Before you start:** `./aws/start-model.sh` (10–15 min; optional — the demo works without it), dev server up, sign in, open the Guided demo (▸ button, or any dashboard URL + `?tour=1`).
...
| 10 | End card (verbal) | Proof stage → launch, in a phased commercial structure. No hardware. No edge appliance. Everything portable. | Don't quote a price beyond what's approved for this engagement. Don't name the existing camera vendor. |
...
- **What's live vs static**: rows with Basis `labelled_history` are the prospect's labels; Basis `model` rows came from a real vision call. Running a kit in chat flips its row to `model` in front of the audience.
