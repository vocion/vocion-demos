#!/usr/bin/env bash
# Run the zero-person-company demo against the pinned vocion-core submodule
# on PORT 3005. Assumes the umbrella's docker-compose Postgres is up (the
# embedded pglite-server's `npm run dev` would otherwise collide on 5432);
# we use `dev:next` to skip that and let the umbrella DB serve every demo.
set -euo pipefail
...
export WORKSPACE_PATH="$DEMO_DIR/workspace/zero-person-company"
...
echo "     with its own vocion_zpc database (see this demo's README)."
...
# lives in the umbrella docker-compose. Using `dev:next` instead of plain
# `dev` lets this demo run side-by-side with every other demo without a
# collision on host 5432.
