 ...
 *
 * Writes two JSONL files under workspace/retail-hiring/data/ (file-import
...
const OUT_DIR = join(dirname(fileURLToPath(import.meta.url)), '..', 'workspace', 'retail-hiring', 'data');
...
const rand = rng(19770101); // fixed seed for deterministic fixture generation
...
const STORES = ['Riverside', 'Lakeside', 'Midtown', 'Harborview', 'Eastgate', 'Westfield'];
...
const L2_HISTORY = [
  ...
  { blurb: '18 months barista at a Harborview coffee shop — espresso bar, grab-and-go case, opening shifts', tags: 'food service + customer-facing' },
...
const WHY = [
  'I shop at Retail Hiring Workforce and love what the store stands for.',
...
function makeApplicant(i, source) {
  ...
  const bodyLines = source === 'indeed'
    ? [
        ...
        `Why Retail Hiring Workforce: ${pick(WHY)}`,
      ]
    : [
        `"Join our Team" application via retailhiring.example.com for ${role}${store ? ` — ${store} store` : ''}.`,
        '',
        `${name} · ${email}`,
...
// Curated fixtures — the three illustrative applicants on the
// proposal's "Harborview manager inbox" page (M. Torres 88 / K. Nakamura 81 /
...
const CURATED = [
  {
    name: 'Marisol Torres', role: 'Deli Clerk', store: 'Harborview',
    blurb: '2 years café and deli counter at a Lakeside coffee shop — prepped the grab-and-go case, ran register on shift rotation; current food-handler card; light on inventory systems',
    availability: 'weekends + evenings', smokes: 'no', ack: 'acknowledged',
    received: '2026-08-11T09:12:00-10:00',
    why: 'I shop at Retail Hiring Workforce and love what the store stands for.',
  },
  {
    name: 'Keone Nakamura', role: 'Deli Clerk', store: 'Harborview',
  ...
  {
    name: 'Jonah Silva', role: 'Deli Clerk', store: 'Harborview',
  ...
  body: [
    ...
    '',
    'RESUME',
    `${c.name} · ${c.name.toLowerCase().replace(/[^a-z]+/g, '.')}@example.com · South Region`,
    `Experience: ${c.blurb}.`,
    ...
    `Why Retail Hiring Workforce: ${c.why}`,
  ].join('\n'),
  ...
  fixture: true,
  curated: 'proposal-p4-harborview-shortlist',
}));
