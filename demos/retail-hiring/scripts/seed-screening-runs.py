...
for a in picks:
    ...
    reviewed = "reviewed_by, reviewed_at", f"'retail@example.com', {ts} + interval '2 hours'"
    ...
    fb_vals = f", '{fb[0]}', '{esc(fb[1])}', 'retail@example.com', {ts} + interval '1 day'" if fb else ""
    print(
        ...
        + f"'approved', 'confident', 'seed:screening', {ts} + interval '10 minutes', 'retail@example.com', {ts} + interval '2 hours'"
        + fb_vals
    ...
    if aid in PENDING_FOLLOWUP:
        fu_out = json.dumps({"applicant": aid, "email_draft": f"Hi {name.split()[0]} — thanks for applying to Retail Hiring Workforce. One thing missing before we can route you: your availability (days/evenings/weekends). Reply to this email and we'll pick your application right back up."})
...
SELECT '{ORG}', '{ORG}', w.id,
  '{{"title": "Jonah Silva — applicant intake", "objectRef": "APP-IN-043", "body": "Application via Indeed for Deli Clerk — Harborview."}}'::jsonb,
  'paused', 3, 'approve step: routing to L. Fonoti · Harborview awaits a person',
