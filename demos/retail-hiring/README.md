...
**AI hiring screener with human review · L2 (score → route → approve) · Retail hiring**
> Two AI coworkers — the **Applicant Screener** and the **Store Router** — read every applicant from Indeed and the website, score them 1–100 against the retailer's own standard, and route qualified candidates (70+) to the right store manager by email. A human approves every step; managers make every hiring decision. Below-threshold applicants are held with the reason recorded — never auto-rejected.

This is the demo instance behind a hiring workforce proposal built for a retail hiring prospect (a regional plant-based grocery and deli chain — six stores, <300 team members, ~60 applicants/month, ~20 hires; a phased monthly commercial structure). It implements the two coworkers from the proposal and the applicant trace it describes, on synthetic data at their real volume, plus the proposal's Agent Registry as inactive teasers. **See `WALKTHROUGH.md` for the presentation script.**

Everything applicant- or manager-shaped in here is an invented fixture. The scoring criteria are demo baselines pending the HR lead's real job descriptions.

## What's in this demo

**Prefer a video?** `assets/retail-hiring-demo.mp4` is a ~3-minute headlessly-rendered walkthrough reel with voiceover and a synthesized music bed (stylized "illustrative interface," watermarked, all data synthetic) — the send-ahead / leave-behind companion to the live demo. Source is `assets/demo-reel.html`; regenerate with `node scripts/record-reel.mjs` (needs playwright + ffmpeg). Frames render deterministically against a stepped virtual clock, so the output is smooth 30fps regardless of machine load. Narration and music live in `scripts/build-reel-audio.mjs`: with `ELEVENLABS_API_KEY` set (env or the umbrella `vocion-local/.env`) the voiceover uses **ElevenLabs** (voice/model via `ELEVEN_VOICE_ID`/`ELEVEN_MODEL`) and the upbeat music bed is **ElevenLabs Music**, generated once and cached at `assets/reel-music.mp3` (delete it or set `REEL_MUSIC_REGEN=1` to regenerate). Without a key it falls back to macOS `say` + a synthesized pad — pass `--silent` to skip audio entirely.

```
demos/retail-hiring/
├── WALKTHROUGH.md                             # the demo script — read this
├── assets/
│   ├── retail-hiring-demo.mp4                    # rendered reel (VO + music)
│   └── demo-reel.html                         # reel source (auto-playing timeline)
├── workspace/retail-hiring/                   # v2 core: "workspace", was "context"
...
│   │       vendor-communications              # inactive — the proposal's Agent Registry teasers
...
│   │   └── retail-web.yaml               # public site crawl (manual sync)
...
2. **Smoking/vaping is a flag, never a filter.** Yes (or blank) on the application's question attaches "CEO approval required per policy" — score unchanged, manager still sees them. Per the HR lead: the CEO reserves this call and almost always approves.
...
`APP-IN-041/042/043` (Marisol Torres, Keone Nakamura, Jonah Silva — all Deli Clerk · Harborview) are hand-authored to reproduce the the proposal's applicant trace and proposal manager-inbox shortlist (target scores ≈88 / ≈81 / ≈74). Run them through `Applicant Intake` when you want the live demo to land exactly on the numbers the HR lead saw in the doc.
...
```bash
docker exec vocion-postgres psql -U postgres -d postgres -c 'CREATE DATABASE vocion_retail_hiring'
docker exec vocion-postgres psql -U postgres -d vocion_retail_hiring -c 'CREATE EXTENSION IF NOT EXISTS vector'
```
...
npm run db:migrate
npm run seed:demo -- \
  --email retail@example.com --password demo123 --name "Retail Hiring Demo" \
  --account-name "Retail Hiring Workforce" \
  --project-slug retail-hiring --project-name "Retail Hiring Workforce"
```

**3. Apply the workspace:**

```bash
export WORKSPACE_PATH=/private/var/www/vocion-local/vocion-demos/demos/retail-hiring/workspace/retail-hiring
...
```bash
cd ../../../demos/retail-hiring
./scripts/dev.sh          # → http://localhost:3003 (sign-in autofills retail@example.com/demo123)
```

Then from `/dashboard/sources`, sync `Indeed` and `Careers — Join our Team` (needs the OpenAI key). Optionally sync `retailhiring.example.com`.

**5. Pre-load the review queue** (so the demo doesn't open on an empty page): run the `Applicant Intake` workflow two or three times from `/dashboard/workflows`, pasting `body` values from `workspace/retail-hiring/data/applicants-indeed.jsonl` — start with the curated `APP-IN-041` (the proposal's own trace), then the WALKTHROUGH cheat sheet. Leave them unapproved.
...
|---|---|
| The pipeline: read → score → route → human approve | Rubric criteria + weights (pending the HR lead's job descriptions) |
...
- Fixture data paths in the source YAMLs are relative to `WORKSPACE_PATH` (that's how `libs/sources/fileImport.ts` resolves them) — hence `data/` living *inside* `workspace/retail-hiring/`.
- This demo is for the live Metacto ↔ Retail Hiring Workforce engagement (data room: `clients/retail-hiring/`). Fixture names are invented; no real applicant or employee data anywhere.
