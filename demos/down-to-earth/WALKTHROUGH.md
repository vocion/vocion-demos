...
`python3 scripts/seed-screening-runs.py | docker exec -i vocion-postgres psql -U postgres -d vocion_dte`
...
- If asked about writes: nothing writes into your systems. ADP gets an export you trigger on hire; **Trackstar has no connection of any kind, on purpose.**

## Beat 3 — The two coworkers (proposal p.3)
...
## Beat 4 — Run an application through, live (proposal p.4, "one applicant, end to end")

- **Dashboard → Workflows → Applicant Intake → Run.** Paste **`APP-IN-041` (Marisol Torres — Deli Clerk, Kailua)** — the fixture authored to reproduce the proposal's own p.4 trace: 2 yrs café + deli counter, food-handler card, weekends, plant-based acknowledged, light on inventory systems → should land ≈88 and route to the Kailua manager.
...
|---|---|---|
| `APP-IN-041` Marisol Torres — Deli Clerk, Kailua | **open with this** | the proposal's own trace: ≈88, explained, routed to Kailua |
| `APP-IN-042` / `APP-IN-043` (Nakamura 81 / Silva 74 profiles) | the shortlist | the full p.5 manager email — three explained candidates, low-70s marked "your call" |
...
| Rideshare driver applying for Store Manager | Amy's own example from the call | held below threshold with the reason recorded — visible, never auto-rejected |
...
## Beat 5 — The review queue (proposal pp.4–5 + p.7 supervision)
...
- Talking point on supervision (p.7): "Everything starts Reviewed — every score and routing waits for your team. When recommendations keep matching your decisions, routine steps run lighter — *your* call, reviewed against the measured match rate. The hiring decision is never automated, at any level."
...
- Close: what's placeholder vs. real — the rubric criteria, the 70 threshold, manager routing, and application wording are demo baselines; setup weeks 1–2 replace them with Down to Earth's real job descriptions in the criteria sessions (the ones Amy is sending inputs for).
...
- **"What does it cost?"** Per the proposal p.10: **$1,500/month all-in on the 24-month term** ($1,750 at 12mo / $1,350 at 36mo — same product). No setup fee (founding-partner terms — worth $6,000), AI usage included to 150 applications/month (more than double today's volume), no per-employee pricing, no usage bill ever. Past 150 for two straight months, the next tier is agreed together in advance. Never quote anything else cold.
...
- **"Does this touch ADP or Trackstar?"** No writes into either. ADP receives hired-candidate details as an export you trigger; Trackstar has no connection of any kind and stays exactly as it is.
- **"Who owns the data / what do we own in year three?"** Down to Earth owns the scoring standard, the applicant data, and the decision history — exportable at any time, with a documented self-host path (the platform core is open source, MPL-2.0). "With a rented ATS the year-three answer is a login; here it's a standard trained by three years of your own decisions."
...
- Scores come from the demo rubric, not Down to Earth's real criteria — say so proactively at Beat 8.
