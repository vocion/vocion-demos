...
2. No preference → do not guess between the five Oahu stores. If application detail makes one store clearly sensible, propose it and say why; otherwise route `preference unknown — Coordinator to ask`. Kahului is the only Maui store.
...
5. Score below 50 → refuse to route; return `below threshold — held` instead of an email.

Store routing targets (DEMO FIXTURES — synthetic manager names, do not present as real people):

| Store | Routes to |
|---|---|
| Kahului (Maui) | K. Nakamura (fixture) — kahului-hiring@downtoearth.example |
...
2. The shortlist email:
   - Subject: `<Role> · <Store> · Score <n> — <band>`
