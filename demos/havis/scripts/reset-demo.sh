#!/usr/bin/env bash
# Reset the Havis demo data to a clean baseline and (optionally) replay the live checks.
#
...
DB=vocion_havis; PSQL="docker exec -i vocion-postgres psql -U postgres -d $DB"
ORG=$($PSQL -tAc "select id from project where slug='havis' limit 1" | tr -d '[:space:]')
[ -n "$ORG" ] || { echo "no project 'havis' in $DB — run seed:demo first"; exit 1; }
BUCKET=$(grep -E '^HAVIS_S3_BUCKET=' "$DEMO/.env.local" | cut -d= -f2)
...
if [ "${1:-}" = "--live" ]; then
  ...
  export WORKSPACE_PATH="$DEMO/workspace/havis" VOCION_REKOGNITION_PROJECT_ARN="${HAVIS_REKOGNITION_PROJECT_ARN:-}"
  (cd "$CORE" && npm run kit:inspect -- --project havis --prefix templates/C-PM-134-PC/bad/ 2>&1 | grep -E "^[✓✗≠?] |agreement")
  (cd "$CORE" && npm run kit:inspect -- --project havis --prefix templates/C-VS-1012-INUT-2-H/bad/ 2>&1 | grep -E "^[✓✗≠?] |agreement")
fi
